...
class Postpone extends StatefulWidget {
  List<Map<String, dynamic>> postponeTasks;
  Postpone(this.postponeTasks);
  @override
  State<Postpone> createState() => _PostponeState(postponeTasks);
}

class _PostponeState extends State<Postpone> {
  List<Map<String, dynamic>> postponeTasks;
  _PostponeState(this.postponeTasks);
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        for(int i=0;i<postponeTasks.length;i++)
          buildTaskCard(postponeTasks[i],Colors.red,() {
            // --- بداية الترتيب التلقائي ---
            setState(() {
              // 1. اعكس حالة المهمة (من صح لغلط أو العكس)
              postponeTasks[i]['isDone'] = !postponeTasks[i]['isDone'];
